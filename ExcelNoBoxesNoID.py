...
#export GOOGLE_APPLICATION_CREDENTIALS=" json file needed from Google Vision
...
for file_name in myListDir(directoryName):
   ...
   # main(fullpath, fullpath[:-4]+"withboxes.jpg",5)
    ...
    for c in colors:
        ...
        sumS = sumS+ c[0]*hsv[1]
        sumV = sumV+ c[0]*hsv[2]
    averageH = sumH/numPixels;
    averageS = sumS/numPixels;
    averageV = sumV/numPixels;
   
    if "Hue" in topRowArr:
        colOfHue = topRowArr.index("Hue")+1
        worksheet.write(row, colOfHue, averageH)
    else:
        topRowArr.append("Hue")
        colOfHue = topRowArr.index("Hue")+1
        worksheet.write(0,colOfHue, "Hue");
        worksheet.write(row, colOfHue, averageH)
    if "Saturation" in topRowArr:
        colOfSat = topRowArr.index("Saturation")+1
        worksheet.write(row, colOfSat, averageS)
    else:
        topRowArr.append("Saturation")
        colOfSat = topRowArr.index("Saturation")+1
        worksheet.write(0,colOfSat, "Saturation");
        worksheet.write(row, colOfSat, averageS)
    if "Value" in topRowArr:
        colOfVal = topRowArr.index("Value")+1
        worksheet.write(row, colOfVal, averageV)
    else:
        topRowArr.append("Value")
        colOfVal = topRowArr.index("Value")+1
        worksheet.write(0,colOfVal, "Value");
        worksheet.write(row, colOfVal, averageV)
    #reset col
    col=0; 
    #row add 1
